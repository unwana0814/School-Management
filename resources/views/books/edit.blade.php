...
<div class="ml-auto bg-gray-200 shadow justify-content-center p-3 mt-5">
    ...
    <form action="{{ route('books.update', $book->id) }}"  style='width:400px' class='w-full max-w-sm' method='POST'>
    @csrf
    @method('PUT')
   
        <div class="flex items-center border-b border-teal-500 py-2">
          <input class="appearance-none bg-transparent border-none w-full text-gray-700 mr-3 py-3 px-2 leading-tight focus:outline-none" type="text" name="author"  placeholder="Author" value="{{ $book->author }}" aria-label="author">
        </div>

        <div class="flex items-center border-b border-teal-500 py-2">
            <input class="appearance-none bg-transparent border-none w-full text-gray-700 mr-3 py-1 px-2 leading-tight focus:outline-none" type="text" name="title" placeholder="Title" value="{{ $book->title }}" aria-label="title">
          </div>

          <div class="flex items-center border-b border-teal-500 py-2">
            <input class="appearance-none bg-transparent border-none w-full text-gray-700 mr-3 py-1 px-2 leading-tight focus:outline-none" type="text" name="year"  placeholder="Year" value="{{ $book->year }}" aria-label="year">
          </div>
